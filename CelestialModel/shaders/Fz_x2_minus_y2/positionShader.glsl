...
// Dz² Orbital ---------------------------------------------------------
float R_4f(float r) {
    return pow(r, 3.0) * exp(-r / (a0));
}

float Y_fz_x2_minus_y2(float theta, float phi) {
    return sin(theta) * cos(theta) * cos(2.0 * phi) * 5.0; // Scaled by 5.0 to match the amplitude of other orbitals
}
...
void main() {
    ...
    float radial = R_4f(r);
    float angular = Y_fz_x2_minus_y2(theta, phi);
    ...
    if (probDensity < 0.01) discard; // or: if (U > probDensity) discard;
