...
float Y_fz3(float theta) {
    float cosTheta = cos(theta);
    return cosTheta * (5.0 * cosTheta * cosTheta - 3.0) * 1.3; // Scaled by 1.3 to match the amplitude of other orbitals
}
...
void main() {
    ...
    if (probDensity < 0.8) discard; // or: if (U > probDensity) discard;
