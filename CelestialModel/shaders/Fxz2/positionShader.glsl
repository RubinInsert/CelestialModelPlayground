...
float Y_fxz2(float theta, float phi) {
    float cosTheta = cos(theta);
    float sinTheta = sin(theta);
    float cosPhi = cos(phi);
    return sinTheta * cosPhi * (5.0 * cosTheta * cosTheta - 1.0) * 2.0; // Scaled by 2.0 to match the amplitude of other orbitals
}
...
void main() {
    ...
    float radial = R_4f(r);
    float angular = Y_fxz2(theta, phi);
