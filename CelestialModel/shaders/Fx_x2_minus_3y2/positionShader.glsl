...
float Y_fx_x2_minus_y2(float theta, float phi) {
    return pow(sin(theta), 3.0) * cos(3.0 * phi) * 2.5; // Scaled by 2.5 to match the amplitude of other orbitals
}
...
void main() {
    ...
    float radial = R_4f(r);
    float angular = Y_fx_x2_minus_y2(theta, phi);
