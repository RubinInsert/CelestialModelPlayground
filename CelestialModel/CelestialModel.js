...
class CelestialModel {
  ...
  static timeStep = 5.0;
  static spinStateFadeFactor = 0.5;
  ...
  constructor(chemSymbol = "H", particleRatio = 1024) {
    ...
    this.spinState = 0.0;
    this.spinStateVisualizationEnabled = false;
  ...
  setSpinStateVisualization(isEnabled) {
    ...
    unpairedElectronsInfo.unpairedOrbitals.forEach((orbit) => {
      let spinStateColor =
        Math.random() < 0.5
          ? new THREE.Color(0xe69f00)
          : new THREE.Color(0x56b4e9); // Randomly choose blue or red
      ...
      if (isEnabled) {
        ...
        this.spinStateVisualizationEnabled = true;
      } else {
        orbitalObjects.forEach((orbitObject) => {
          if (orbitObject.particles && orbitObject.particles.material) {
            orbitObject.particles.material.fragmentShader =
