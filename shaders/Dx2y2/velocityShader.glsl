...
const float scaleFactor = 2.0; // Arbitrary scaling factor because Dx2y2 uses a different estimation function
bool isInsideDxOrbital(vec3 point, float threshold) {
    float r = length(point); // r = sqrt(x^2 + y^2 + z^2)
    float psiSquared = pow(point.x * point.x - point.y * point.y, 2.0) * exp(-2.0 * r);
...
vec3 gradientPsiSquared(vec3 point) {
    ...
    // Partial derivatives of (x^2 - y^2)^2 * exp(-2r)
    float dPsiDx = 4.0 * (point.x * point.x - point.y * point.y) * point.x * exp(-2.0 * r)
                   - 2.0 * psiSquared * point.x / r;
    float dPsiDy = -4.0 * (point.x * point.x - point.y * point.y) * point.y * exp(-2.0 * r)
                   - 2.0 * psiSquared * point.y / r;
    float dPsiDz = -2.0 * psiSquared * point.z / r; // Optional, but typically zero for Dx orbital
    ...
    void main() {
        ...
        if(!isInsideDxOrbital(position.xyz, threshold * scaleFactor)) {
