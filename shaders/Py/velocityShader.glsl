...
bool isInsidePyOrbital(vec3 point, float threshold) {
    float r = length(point); // r = sqrt(x^2 + y^2 + z^2)
    float psiSquared = point.y * point.y * exp(-2.0 * r); // Py orbital probability density
    return psiSquared > threshold;
}

vec3 gradientPsiSquared(vec3 point) {
    float r = length(point);
    float psiSquared = point.y * point.y * exp(-2.0 * r);

    // Partial derivatives of psiSquared with respect to x, y, and z
    float dPsiDx = -2.0 * point.y * point.y * point.x * exp(-2.0 * r) / r;
    float dPsiDy = 2.0 * point.y * exp(-2.0 * r) - 2.0 * point.y * point.y * point.y * exp(-2.0 * r) / r;
    float dPsiDz = -2.0 * point.y * point.y * point.z * exp(-2.0 * r) / r;
    ...
    void main() {
        ...
        if(!isInsidePyOrbital(position.xyz, threshold)) {
