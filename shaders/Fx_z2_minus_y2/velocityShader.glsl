...
// Angular part: x * (z^2 - y^2)
float angular = point.x * (point.z * point.z - point.y * point.y);
float psiSquared = angular * angular * exp(-2.0 * r);
return psiSquared > threshold;
}

// Computes the gradient of psiSquared = [x*(z^2 - y^2)]^2 * exp(-2r)
// We write it as: psiSquared = P * exp(-2r), with P = x^2 * (z^2 - y^2)^2.
...
// Define Q = (z^2 - y^2)^2 and P = x^2 * Q.
float Q = (point.z * point.z - point.y * point.y) * (point.z * point.z - point.y * point.y);
float P = point.x * point.x * Q;

// Compute partial derivatives of P:
// dP/dx = 2*x * Q
float dPdx = 2.0 * point.x * Q;
// For y: d/dy (z^2 - y^2)^2 = 2*(z^2 - y^2)*(-2y) = -4*y*(z^2 - y^2)
float dPdy = point.x * point.x * (-4.0 * point.y * (point.z * point.z - point.y * point.y));
// For z: d/dz (z^2 - y^2)^2 = 2*(z^2 - y^2)*(2z) = 4*z*(z^2 - y^2)
float dPdz = point.x * point.x * (4.0 * point.z * (point.z * point.z - point.y * point.y));
...
return vec3(dPsiDx, dPsiDy, dPsiDz);
    ...
    void main() {
        ...
        if(!isInsideFxOrbital(position.xyz, threshold)) {
