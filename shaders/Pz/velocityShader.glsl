...
bool isInsidePzOrbital(vec3 point, float threshold) {
    float r = length(point); // r = sqrt(x^2 + y^2 + z^2)
    float psiSquared = point.z * point.z * exp(-2.0 * r); // Pz orbital probability density
    return psiSquared > threshold;
}

vec3 gradientPsiSquared(vec3 point) {
    float r = length(point);
    float psiSquared = point.z * point.z * exp(-2.0 * r);

    // Partial derivatives of psiSquared with respect to x, y, and z
    float dPsiDx = -2.0 * point.z * point.z * point.x * exp(-2.0 * r) / r;
    float dPsiDy = -2.0 * point.z * point.z * point.y * exp(-2.0 * r) / r;
    float dPsiDz = 2.0 * point.z * exp(-2.0 * r) - 2.0 * point.z * point.z * point.z * exp(-2.0 * r) / r;
    ...
    void main() {
        ...
        if(!isInsidePzOrbital(position.xyz, threshold)) {
