...
bool isInsideDyzOrbital(vec3 point, float threshold) {
    float r = length(point); // r = sqrt(x^2 + y^2 + z^2)
    float psiSquared = pow(point.y * point.z, 2.0) * exp(-2.0 * r); // Use yz instead of xz
    return psiSquared > threshold;
}

vec3 gradientPsiSquared(vec3 point) {
    float r = length(point);
    float psiSquared = pow(point.y * point.z, 2.0) * exp(-2.0 * r);

    // Partial derivatives of (yz)^2 * exp(-2r)
    float dPsiDx = -2.0 * psiSquared * point.x / r; // No dependence on x for Dyz orbital
    float dPsiDy = 2.0 * point.y * point.z * point.z * exp(-2.0 * r)
                   - 2.0 * psiSquared * point.y / r;
    float dPsiDz = 2.0 * point.z * point.y * point.y * exp(-2.0 * r)
    ...
    void main() {
        ...
        if(!isInsideDyzOrbital(position.xyz, threshold)) { // THESHOLD is half the size to account for the smaller value it produces
